...
This sub-module (`mq/pulsar`) is being phased out in favor of a unified `mq` module.

**Old import** (v1.x - Deprecated):
```go
import "github.com/astra-go/astra/mq/pulsar"

p, err := pulsar.NewProducer(pulsar.ProducerConfig{
...
p, err := mq.NewProducer("pulsar", mq.ProducerOptions{
    Brokers: []string{"localhost:9092"},
})
```
