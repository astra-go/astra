...
This sub-module (`mq/rabbitmq`) is being phased out in favor of a unified `mq` module.

**Old import** (v1.x - Deprecated):
```go
import "github.com/astra-go/astra/mq/rabbitmq"

p, err := rabbitmq.NewProducer(rabbitmq.ProducerConfig{
...
p, err := mq.NewProducer("rabbitmq", mq.ProducerOptions{
    Brokers: []string{"localhost:9092"},
})
```
