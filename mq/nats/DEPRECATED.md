...
This sub-module (`mq/nats`) is being phased out in favor of a unified `mq` module.

**Old import** (v1.x - Deprecated):
```go
import "github.com/astra-go/astra/mq/nats"

p, err := nats.NewProducer(nats.ProducerConfig{
...
p, err := mq.NewProducer("nats", mq.ProducerOptions{
    Brokers: []string{"localhost:9092"},
})
```
