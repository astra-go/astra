...
//
// An Elasticsearch 8 container is started once via TestMain and torn down
// after all tests complete.  No external container or environment variable is
// required — just run:
//
//	go test -tags integration -v ./search/elastic/...
//
// Docker (or a compatible runtime) must be available on the test host.
...
// containerAddr / containerUser / containerPass / containerCACert are set by
...
// TestMain starts a single Elasticsearch 8 container for the entire test run.
...
// newClient creates an elastic.Client pointed at the test container.
...
// uniqueIndex returns a unique index name to avoid cross-test pollution.
func uniqueIndex(t *testing.T) string {
	t.Helper()
	return fmt.Sprintf("astra-test-%d", rand.Int63())
}

// withIndex creates an index, registers cleanup, and returns its name.
...
// refresh waits for ES to make recently indexed docs visible to searches.
// ES8 containers under test are single-node; a fixed sleep is the simplest
// approach without the refresh API.
...
// TestIntegration_CreateIndex_WithMapping verifies that a typed mapping is
// accepted and that the index can be used for keyword searches immediately.
func TestIntegration_CreateIndex_WithMapping(t *testing.T) {
	...
	// A second CreateIndex on the same name must be rejected.
...
// TestIntegration_Index_Overwrite verifies that re-indexing the same ID
// replaces the document (ES "upsert" semantics).
...
// TestIntegration_Delete_NonExistent verifies that deleting a non-existent
// document ID does not return an error (404 is silently ignored).
...
func TestIntegration_Search_Aggregation(t *testing.T) {
	...
	// Buckets are nested maps from the raw JSON; just verify the key exists.
